...
Class Receipt extends CI_Model{
    ...
    function getunpaid($params){
        $sql = ' select a.id,a.name,a.cp,a.address,a.phone,a.district from locations a ';
        $sql.= ' left outer join (select id from receipts where monthyear="'.$params['monthyear'].'") b on b.id=a.id ';
        $sql.= ' where b.id is null ';
//        $sql.= ' and b.monthyear="'.$params['monthyear'].'"';
       // $sql.= ' and b.monthyear!='.$params['monthyear'];
        $ci = & get_instance();
        $que = $ci->db->query($sql);
        return array(
